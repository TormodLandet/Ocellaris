...
def taylor_to_DG2_matrix_3D(V):
    ...
    for cell in cells(mesh):
        ...
        bar_xx = (-3*x1 + x2 + x3 + x4)**2/3840 + (x1 - 3*x2 + x3 + x4)**2/3840 + (x1 + x2 - 3*x3 + x4)**2/3840 + (x1 + x2 + x3 - 3*x4)**2/3840
        bar_yy = (-3*y1 + y2 + y3 + y4)**2/3840 + (y1 - 3*y2 + y3 + y4)**2/3840 + (y1 + y2 - 3*y3 + y4)**2/3840 + (y1 + y2 + y3 - 3*y4)**2/3840
        bar_zz = (-3*z1 + z2 + z3 + z4)**2/3840 + (z1 - 3*z2 + z3 + z4)**2/3840 + (z1 + z2 - 3*z3 + z4)**2/3840 + (z1 + z2 + z3 - 3*z4)**2/3840
        bar_xy = x1*y1/160 - x1*y2/480 - x1*y3/480 - x1*y4/480 - x2*y1/480 + x2*y2/160 - x2*y3/480 - x2*y4/480 - x3*y1/480 - x3*y2/480 + x3*y3/160 - x3*y4/480 - x4*y1/480 - x4*y2/480 - x4*y3/480 + x4*y4/160
        bar_xz = x1*z1/160 - x1*z2/480 - x1*z3/480 - x1*z4/480 - x2*z1/480 + x2*z2/160 - x2*z3/480 - x2*z4/480 - x3*z1/480 - x3*z2/480 + x3*z3/160 - x3*z4/480 - x4*z1/480 - x4*z2/480 - x4*z3/480 + x4*z4/160
        bar_yz = y1*z1/160 - y1*z2/480 - y1*z3/480 - y1*z4/480 - y2*z1/480 + y2*z2/160 - y2*z3/480 - y2*z4/480 - y3*z1/480 - y3*z2/480 + y3*z3/160 - y3*z4/480 - y4*z1/480 - y4*z2/480 - y4*z3/480 + y4*z4/160
...
def produce_code_with_sympy_3D(degree):
    ...
    (Only used for generating the code above, not used while running Ocellaris)
    """
    from sympy import Symbol, symbols, S, sqrt
    ...
    if degree == 1:
        ...
        print('# Cell average value')
        print('A[icell, 0, 0] = -1/120')
        print('A[icell, 0, 1] = -1/120')
        print('A[icell, 0, 2] = -1/120')
        print('A[icell, 0, 3] = -1/120')
        print('A[icell, 0, 4] = 1/30')
        print('A[icell, 0, 5] = 1/30')
        print('A[icell, 0, 6] = 1/30')
        print('A[icell, 0, 7] = 1/30')
        print('A[icell, 0, 8] = 1/30')
        print('A[icell, 0, 9] = 1/30\n')
        ...
        a = 1/(3*sqrt(5) - 5)
        b = 1/(1*sqrt(5) + 5)
        BT_QUADRATURE_TABLE[1] = ([[I/4, I/4, I/4]], [I/6])
        BT_QUADRATURE_TABLE[2] = ([[a, b, b], [b, a, b], [b, b, a], [b, b, b]], [I/24, I/24, I/24, I/24])
        BT_QUADRATURE_TABLE[3] = ([[I/4, I/4, I/4],
                                   [I/2, I/6, I/6],
                                   [I/6, I/2, I/6],
                                   [I/6, I/6, I/2],
                                   [I/6, I/6, I/6]],
                                  [S(-2)/15, S(3)/40, S(3)/40, S(3)/40, S(3)/40])
