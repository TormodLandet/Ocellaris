...
class BlendedField(KnownField):
    ...
    def read_input(self, field_inp):
        ...
        verify_key('blending field0', field_name0, self.simulation.fields, 'BlendedField %s' % self.name)
        verify_key('blending field1', field_name1, self.simulation.fields, 'BlendedField %s' % self.name)
        blend = verify_field_variable_definition(self.simulation, blend_def, 'BlendedField %s' % self.name)
