...
class ConvectionSchemeHric2D(ConvectionScheme):
    description = 'High Resolution Interface Capturing (HRIC, Modified HRIC and Re'
    ...
    def update(self, t, dt, velocity):
        ...
        for facet in dolfin.facets(self.mesh):
            ...
            aU = aC + numpy.dot(gC, vec_to_upstream)
            aU = max(aU, nminC)
            aU = min(aU, nmaxC)
            ...
            # Approximate gradient at the interface
            g = 0.5*(gC + gD)
            len_g2 = g[0]**2 + g[1]**2
