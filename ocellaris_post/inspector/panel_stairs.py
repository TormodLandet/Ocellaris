...
class OcellarisStairsPanel(wx.Panel):
    ...
    def layout_widgets(self):
        ...
        L = 0
        gbs.Add(wx.StaticText(self, label='Control report:'), flag=wx.ALIGN_CENTER_VERTICAL, pos=(L, 0), span=(1, 1))
        self.control_selector = wx.Choice(self)
        self.control_selector.Bind(wx.EVT_CHOICE, self.control_selected)
        gbs.Add(self.control_selector, flag=wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, pos=(L, 1), span=(1, 1))
        
        gbs.Add(wx.StaticText(self, label='Control value:'), flag=wx.ALIGN_CENTER_VERTICAL, pos=(L, 2), span=(1, 1))
        self.control_value = wx.Choice(self)
        self.control_value.Bind(wx.EVT_CHOICE, self.control_value_selected)
        gbs.Add(self.control_value, flag=wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, pos=(L, 3), span=(1, 1))
        
        L += 1
        gbs.Add(wx.StaticText(self, label='Plot report:'), flag=wx.ALIGN_CENTER_VERTICAL, pos=(L, 0), span=(1, 1))
        self.report_selector = wx.Choice(self)
        self.report_selector.Bind(wx.EVT_CHOICE, self.report_selected)
        gbs.Add(self.report_selector, flag=wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, pos=(L, 1), span=(1, 3))
...
def get_stairs(all_results, report_name):
    """
    Given a list of Result objects, return two lists
    
    - all_vals: all discovered flat values in the report_name time series
                in all_results merged into one ordered list (unique vals)
    
    - all_lims: same length as all_vals; each item is a list of the same
                length as all_results and each list item is a tuple of
                start and end indices of the flat part; or (0, 0, 0)
                if there was no such flat part in that result object
