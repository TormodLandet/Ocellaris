...
def plot_backward_facing_step(res):
    ...
    x_inflow = -1.5*H1
    _, y_inflow, _, u0_inflow = get_probe_values(u[0], [x_inflow, 0, 0], [x_inflow, H1, 0], 20)
    ...
    levels = get_probe_values(sf.psi, [1.5*H2, -0.5*H2, 0], [H2, H1*0.99, 0], N_levels)[-1]
    levels2 = []
    ...
    ax.tricontour(triangulation, Z, levels, linestyles='solid', colors='#0000AA', linewidths=0.5)
    ...
    ax.set_title('Re = %g, t = %g, xi = %s' % (Re, time, ', '.join('%.3f' % e[0] for e in x1_c0)))
    fig.tight_layout()
