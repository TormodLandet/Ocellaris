...
class X3DMesh(object):
    ...
    def _write(self, scene_element):
        ...
        ifs.set(
            'coordIndex',
            ' -1 '.join(' '.join(str(v) for v in vs) for vs in self.connectivity)
            + ' -1',
        )
        coords.set(
            'point', ' '.join(' '.join(str(x) for x in xs) for xs in self.coordinates)
        )
...
class X3DFile(object):
    ...
    def write(self, x3d_file_name):
        ...
        SubElement(
            head,
            'meta',
            {'name': 'generator', 'content': 'Ocellaris iso_line_to_xd3.py'},
        )
...
def iso_line_to_x3d(iso_file_name, x3d_file_name):
    ...
    print(
        'Processing field %s with iso-line at %g, dim = %r' % (field_name, value, dim)
    )
